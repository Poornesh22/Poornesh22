                                ...
                                <tr key={day}>
                                    ...
                                    {values.map((allvalues, j) => (
                                        <td key={j} className="border border-gray-500 px-1 py-1">
                                            <div className="flex flex-col w-48 min-h-20 text-xs">
                                                {allvalues.map((val, x) => (
                                                    <div key={x}>
                                                        {[...Array(val.length)].map((_, i) => ((i + 1) % 5 == 0 ? <><span key={i}>{val[i]}</span><div className='w-48 bg-gray-600 h-[0.5px] p-0'></div></> : <span key={i}>{val[i]}/</span>))}
                                                    </div>
                                                ))}
                                            </div>
                                        </td>
        ...
        <div className="mt-5 flex items-center justify-center bg-gray-100 p-6">
            <div className="w-full p-6 bg-amber-300 border-2 border-black rounded-lg shadow-lg flex flex-col justify-normal overflow-x-scroll scrollbar-thin scrollbar-thumb-amber-300 scrollbar-track-transparent">
                ...
                <div className="mb-4">
                    ...
                    <select onChange={(e) => setstval(e.target.value)} onClick={() => getdata("stream", "stream", "First select a stream")} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
