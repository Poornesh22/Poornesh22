                                ...
                                <tr key={day}>
                                    ...
                                    {values.map((allvalues, j) => (
                                        <td key={j} className="border border-gray-500 px-1 py-1">
                                            <div className="flex flex-col min-w-56 min-h-20 text-xs">
                                                {allvalues.map((val, x) => (
                                                    <div key={x}>
                                                        {[...Array(val.length)].map((_, i) => ((i+1) % 4 == 0 ? <><span key={i}>{val[i]}</span><div className='w-56 bg-gray-600 h-[0.5px] p-0'></div></>:<span key={i}>{val[i]}/</span>))}
                                                    </div>
                                                ))}
                                            </div>
                                        </td>
