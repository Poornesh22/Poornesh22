        ...
        <div className="mt-1 flex items-center justify-center bg-gray-100 p-1">
            <div className="w-full p-6 bg-amber-300 border-2 border-black rounded-lg shadow-lg flex flex-col justify-normal overflow-x-scroll scrollbar-thin scrollbar-thumb-amber-300 scrollbar-track-transparent">
                ...
                <div className="mb-4">
                    ...
                    <select onChange={(e) => setseval(e.target.value)} onClick={() => getdata("semester", `S${stval}`, "First select a stream")} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        {semester1.map(ch => <option key={ch} value={ch} >{ch}</option>)}
                    </select>
                </div>

                <button
                    onClick={() => gettable("semester_table", [`S${stval}`,seval], "First select a Department")}
